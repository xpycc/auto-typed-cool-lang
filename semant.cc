...
#define SEMANT_ERROR(node) semant_error(c->get_filename(), node)
  ...
  for (int i = fea->first(); fea->more(i); i = fea->next(i)) {
    ...
    if (typeid(*f) == typeid(method_class)) {
      ...
      for (int j = formals->first(); formals->more(j); j = formals->next(j)) {
        ...
        if (ff->get_name() == self) {
          SEMANT_ERROR(ff) << "Method cannot have any formal parameter named self.\n";
        }
        if (sign[j] == SELF_TYPE) {
          SEMANT_ERROR(ff) << "Formal parameter '" << ff->get_name() << "' cannot have type SELF_TYPE.\n";
        }
        if (formal_set.find(ff->get_name()) != formal_set.end()) {
          SEMANT_ERROR(ff) << "Formal parameter '" << ff->get_name()
      ...
      if (curr_sign.find(meth->get_name()) != curr_sign.end()) {
        SEMANT_ERROR(meth) << "Redefinition of method '" << meth->get_name() << "' in class " << super << ".\n";
      } else if (pare_sign.find(meth->get_name()) != pare_sign.end() && pare_sign[meth->get_name()] != sign) {
        SEMANT_ERROR(meth) << "Definition of override method '" << meth->get_name() << "' in class " << super
      ...
      if (attr->get_name() == self) {
        SEMANT_ERROR(attr) << "Class cannot have any attribute named self.\n";
      } else if (curr_attr.find(attr->get_name()) != curr_attr.end()) {
        SEMANT_ERROR(attr) << "Redefinition of attribute '" << attr->get_name() << "' in class " << super << ".\n";
      } else if (pare_attr.find(attr->get_name()) != pare_attr.end()) {
        SEMANT_ERROR(attr) << "Attribute '" << attr->get_name() << "' in class "
...
void ClassTable::check_class_members() {
  ...
  for (map<Symbol, map<Symbol, Signature> >::iterator i = signature_map.begin(); i != signature_map.end(); ++i) {
    ...
    for (map<Symbol, Signature>::iterator j = i->second.begin(); j != i->second.end(); ++j) {
      ...
      for (size_t k = 0; k + 1 < j->second.size(); ++k) {
...
#undef SEMANT_ERROR
#define SEMANT_ERROR(node) classtable->semant_error(curr_class->get_filename(), node)
    ...
    for (int j = fea->first(); fea->more(j); j = fea->next(j)) {
      ...
      if (typeid(*f) == typeid(method_class)) {
        ...
        if (!new_check_if_A_is_B(meth->get_expr()->check_type(), meth->get_return_type())) {
          SEMANT_ERROR(meth) << "Inferred method body expression type " << meth->get_expr()->get_type()
        ...
        if (init_type != No_type && !new_check_if_A_is_B(init_type, attr->get_type_decl())) {
          SEMANT_ERROR(attr) << "Inferred attribute initializing expression type " << attr->get_init()->get_type()
...
#undef SEMANT_ERROR
#define SEMANT_ERROR classtable->semant_error(curr_class->get_filename(), this)

Symbol assign_class::check_type() {
  Symbol id_type = symboltable->lookup(name),
      expr_type = expr->check_type();
  if (name == self) {
    SEMANT_ERROR << "Cannot assign object to self.\n";
    return type = expr_type;
  }
  if (id_type == NULL) {
    SEMANT_ERROR << "Undefined identifier " << name << ".\n";
    id_type = Object;
  }
  if (!new_check_if_A_is_B(expr_type, id_type)) {
    SEMANT_ERROR << "Inferred expression type " << expr_type
...
Symbol dispatch_class::check_type() {
  ...
  if (sign.size() == 0) {
    SEMANT_ERROR << "Dispatch to undefined method " << name << " for class " << disp_type << ".\n";
    return type = Object;
  }
  ...
  if (sign.size() != actual_type.size() + 1) {
    SEMANT_ERROR << "Unmatched numbers of actual and formal parameters in dispatching " << name << ".\n";
    return type;
  }
  for (size_t i = 0; i < actual_type.size(); ++i) {
    if (!new_check_if_A_is_B(actual_type[i], sign[i])) {
      SEMANT_ERROR << "The inferred " << i + 1 << "th actual parameter type " << actual_type[i]
...
Symbol static_dispatch_class::check_type() {
  ...
  if (type_name == SELF_TYPE) {
    SEMANT_ERROR << "Static dispatching type cannot be SELF_TYPE.\n";
    return type = Object;
  }
  ...
  if (sign.size() == 0) {
    SEMANT_ERROR << "Dispatch to undefined method " << name << " for class " << type_name << ".\n";
    return type = Object;
  }
...
Symbol cond_class::check_type() {
  ...
  if (pred_type != Bool) {
    SEMANT_ERROR << "Predictive expression type must be Bool in 'if-else' expressions.\n";
  }
...
Symbol loop_class::check_type() {
  if (pred->check_type() != Bool) {
    SEMANT_ERROR << "Predictive expression type must be Bool in 'while-loop' expressions.\n";
  }
  body->check_type();
  return type = Object;
}
...
Symbol let_class::check_type() {
  ...
  if (init_type != No_type && !new_check_if_A_is_B(init_type, type_decl)) {
    SEMANT_ERROR << "The inferred initializing expression type " << init_type
...
Symbol new__class::check_type() {
  if (type_name != SELF_TYPE && classtable->find_class(type_name) == NULL) {
    SEMANT_ERROR << "Undefined type " << type_name << " for 'new' expresssions.\n";
    return type = Object;
  } else {
